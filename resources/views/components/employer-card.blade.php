...
<x-employer-panel
    class="relative min-h-[250px] items-center">
    ...
    <div>
        ...
        @else
            <a href="{{ url('/jobs/employer/'.$employer->id) }}"
    ...
    <x-forms.form id="delete-employer-{{ $employer->id }}" method="DELETE" action="{{ url('/employer/'.$employer->id) }}">
        <div class="mt-4 flex text-xs text-white">
            <div
                ...
                <a href="{{ url('/employer/'.$employer->id.'/edit') }}" class="">Edit</a>
            </div>
